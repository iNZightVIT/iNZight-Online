...
################################################################
# functions used across tools in iNZight
################################################################
...
getdata <- reactive({
  ...
  # getdata reactive is changing the character type to factor type
...
getdata_class <- reactive({
	...
	cls <- gsub("ordered","factor", cls)
  
	cls[cls %in% c("numeric","integer") ] <- "num"
	cls[cls %in% c("character","factor")] <- "cat"
	cls[cls %in% c("logical","complex")] <- "undefined"
  # print(cls)
  cls
})
